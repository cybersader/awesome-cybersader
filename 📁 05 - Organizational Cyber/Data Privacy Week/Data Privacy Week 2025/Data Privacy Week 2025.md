---
aliases: []
tags: []
publish: true
permalink:
date created: Friday, January 24th 2025, 3:46 pm
date modified: Monday, January 27th 2025, 11:46 am
---

%% Begin Landmark %%
- **[[Data Privacy Week 2025 Webinar]]**
- **[[Data Privacy Week Day 1]]**

%% End Landmark %%

# Data Privacy Week Content

## Data Privacy Week

### Day 1) Intranet Post

#### D1) Article

```
# It's Not About Conspiracies and "Creepiness"

When most people hear “data privacy,” they picture folks taping over their webcams or panicking about government wiretaps. And inevitably, someone says, “I’ve got nothing to hide—why should I care?” But modern data privacy isn’t about covering up nefarious secrets; it’s about **maintaining control** over your life in a world where _everything_ can be recorded, aggregated, and analyzed in ways that hurt your goals or financial dreams.

We spend **massive amounts of our daily lives** on the internet—whether browsing, shopping, working, or connecting with friends. And because technology evolves so rapidly, today’s “harmless” data could be tomorrow’s goldmine for corporations, adversaries, or rogue third parties. _That_ is why data privacy matters.

# What _Is_ Data Privacy?

At its core, data privacy is about **control**: who has your information, how they obtained it, and what they plan to do with it. It’s also about **protecting your interests**—your goals, your values—and making sure they don’t get subverted by powerful entities with opposing aims. Finally, it’s about **consent**. You shouldn’t be forced to trade away your personal data without knowing why or how it will be used.

# Old Privacy vs. Modern Data Privacy

- **Old Privacy:** Think of the constitutional right to be free from unreasonable searches. It was mostly about physical barriers—your home, your property, and your person.
- **Modern Data Privacy:** Today, _digital_ footprints tell a more revealing story than a simple peek through your mailbox or search of your person ever could. Every click, search, and app use can be tracked, compiled, and mined for insights about your shopping habits, political leanings, mental health, and more. This isn’t just about what you _intentionally_ share—it’s also about what’s collected in the background, often without you realizing it.

# Data Is Power

We’ve generated more data in the last few years than in all of human history before that. Why is that a big deal? Because **data = power**. Organizations that aggregate and analyze data on a massive scale wield extraordinary influence:

- **Personal Manipulation:** If advertisers know precisely how to grab your attention, they can steer your decisions—what you buy, which headlines you click, and even how you vote. Not to mention, every distraction can sap valuable time and attention. Essentially, we live in a world where _you_ (and your future decisions) become the product.
- **Societal Impact:** Large-scale data collection can polarize communities by micro-targeting ads or political messages. The result? Echo chambers and misinformation. This wouldn’t be as much of an issue if data privacy were more robust and transparent.
- **Governmental Overreach:** Historical examples show that benign data collection can morph into surveillance that undermines free speech and civil liberties. Also, other nation states can use data to attack our government in any number of ways.

And yet, the harm usually creeps in slowly. Humans are wired to notice immediate threats—a phishing email, for example—but not the long-term risks of data misuse. It’s like the “boiling frog” scenario: the temperature rises degree by degree, and by the time we realize something’s wrong, it’s too late.
Crucially, it all boils down to **power dynamics**: Who holds the data, how is it being used, and are those uses aligned with _your_ values—or working against them?

# What It _Isn’t_ About

- **Zero Public Data:** Data privacy doesn’t mean erasing your online presence or refusing to share any personal info.
- **Stopping Legitimate Uses:** We still want businesses and law enforcement to access certain data when it aligns with our collective interests (solving crimes, preventing fraud, etc.).
- **Massive Compliance Burdens:** Yes, regulations can be costly, but the goal is fair, transparent usage—_not_ to grind business to a halt. However, businesses should only be required to do what’s **proportional** to protecting user privacy.

# Three Levels of Concern

1. **Personal:**
    - Your autonomy. Even “harmless” info like favorite cereals or frequent travel spots can be pieced together to influence or exploit you.
    - Your future opportunities. Today’s data might impact your job prospects, insurance rates, or credit score years from now.
2. **Societal (Including Businesses):**
	- Businesses with goals opposed to your own can have more power. Your data can be used in a pool of other people's data to give those businesses more power.
    - Trust is critical. If people realize companies misuse data, faith in entire industries erodes.
    - Polarization. Targeted ads and social media algorithms can deepen divides, making it easier to manipulate public opinion.
3. **Governmental:**
	- Other countries using data about our citizens to erode societal interests.
    - Mass Surveillance. When entire populations can be tracked remotely at scale, privacy erodes, chilling free speech and dissent.
    - Checks & Balances. Without strong privacy protections, governments can collect and analyze data for ends that may conflict with collective interests at that level.

# Bottom Line

- **Privacy is about choice.** If you don’t control who holds your data and why, _they_ get to shape your narrative.
- **Data is power.** An entity with enough data about you can influence your decisions without your explicit awareness.
- **Your values matter.** Data can be shared, sold, or exploited in ways that clash with what you believe in—even if you never see it happening.
- **Small leaks accumulate.** A single data point might be harmless, but a thousand tiny data points create a detailed (and potentially exploitable) portrait of your life.

So yes, you _do_ have something to hide. That “something” is your right to pursue your own goals—rather than unknowingly fueling someone else’s profit motives. By staying alert to how data is gathered and used—and by demanding transparency and consent—you safeguard not just your personal interests, but our collective freedom to shape the future on our own terms.

**Remember:** Data privacy doesn’t mean living off the grid. It’s about **balancing** openness, convenience, and autonomy—sharing information thoughtfully rather than automatically. The more **control** you have over your data, the more control you have over your life.
```

#### D1) Outline w/some phrases

- Data Privacy - why you *actually* have something to hide
- Hook
	- What most people picture
		- When most people hear “data privacy,” they picture someone covering their webcam or worrying about government conspiracies. And inevitably, someone says, “I’ve got nothing to hide—why should I care?” But modern data privacy isn’t about hiding shady activities.
	- What it's actually about
		- It’s about maintaining control over your life in a world where _everything_ can be recorded, aggregated, and analyzed.
- What is Data Privacy
	- It's about control
	- It's about protecting interests
	- It's about consent
- Old Privacy vs Modern Data Privacy
	- Old Privacy
		- Constitution?
		- What did old privacy entail or examples
	- Modern Data Privacy
		- As the internet becomes an integral part of our lives, entities on the internet take up tons of data about people
- Societal Advancements (optional tidbit to add in somewhere)
	- Examples from history
	- The fundamental conclusion
- Data is a new power
	- What it's not about
		- Not about seeing every little way that a company uses your data.  This is the issue with some data privacy enforcement is that it creates large costs for business.
		- It's not about having no public data on yourself and definitely not about censoring people.
		- 
	- We are bad at seeing long-term risks
		- We can’t foresee all the ways data might be used.  Technology evolves rapidly. Today’s harmless info can become tomorrow’s goldmine for manipulation—by corporations, adversaries, or rogue third parties.
		- One of the biggest challenges with data privacy is that the _harm_ often appears in small ways, over a long period of time. We humans are wired to notice immediate threats—like an email scam in your inbox—but the real danger can creep in silently over months or years. History is full of examples where personal data or records got collected for one benign purpose and ended up being used for something far worse down the line.
	- What it is about
		- It’s about power dynamics—who holds the data, how that data gets used, and whether it aligns with _your_ interests.
		- Notice that it's about control over who has your data and how they can use it.  Granted, you can't have too much knowledge about a company's "secret sauce" but you should know what goal it's being used for.
		- Even if the information is technically public, the act of aggregating, analyzing, and exploiting it at scale is where the harm lies. Transparency ensures you’re aware of _how_ your data is used, while control gives you the power to say yes or no.  It's about interest alignment.  It's about someone having the freedom to life, liberty, and the pursuit of happiness without being deceptively exploited by entities with opposing interests.
	- If you have goals, then you should care
	- ...but how much to care and when?
		- So whats the problem with someone having our data and who are the "someones" that are risky?
- Three Levels of Concern
	- Personal
	- Societal / Business
	- Governmental
- Bottom line
	- **Privacy is about choice**: If you don’t control who has your data, _they_ control your narrative (and possibly your future).
	- Data is power.
	- **Your values matter**: Data can be sold, shared, or analyzed in ways that directly oppose your personal or organizational values—even if you never see it happening.
	- 

#### D1) Content

Data Privacy - why you *actually* have something to hide

This is the most common argument you hear when people start talking about data privacy and it's usually the reaction to some comments about Chinese espionage, talks about VPNs, or the result of what FUD media is pushing lately about technology.  We are going to cut through all of that this week so you can know what to actually worry about and how to think about data privacy in a simple way.

So, What *Is* Data Privacy?

Data privacy is about your ability to control _who_ has information about you and _how_ they use it. It’s not just about hiding embarrassing photos or paranoid government conspiracies. ==It’s about power dynamics—who holds the data, how that data gets used, and whether it aligns with _your_ interests.==

Why Should *You* Care?

- ==**Data can be used to hurt your goals.**==
    Even if you live a squeaky-clean life, having every search query, purchase, or click tracked and compiled could eventually be leveraged in ways you never intended. ==Think personalized persuasion that pushes you away from your true goals.==

- **Data can be used to hurt our members.**
    In our line of work, trust is everything. If malicious actors gain insights into our members’ activities, it can damage their interests and our reputation in one swoop.

- **Data can be used to hurt our mission.**
    Whether it’s shaping narratives, selling off insights, or simply weaponizing information to stall our projects—having the wrong people in control of our data can sabotage our overarching goals as an organization.

==Notice that it's about control over who has your data and how they can use it.  Granted, you can't have too much knowledge about a company's "secret sauce" but you should know what goal it's being used for.==

==Even if the information is technically public, the act of aggregating, analyzing, and exploiting it at scale is where the harm lies. Transparency ensures you’re aware of _how_ your data is used, while control gives you the power to say yes or no.==

==So whats the problem with someone having our data and who are the "someones" that are risky?==

We are bad at seeing long-term dangers

One of the biggest challenges with data privacy is that the _harm_ often appears in small ways, over a long period of time. We humans are wired to notice immediate threats—like an email scam in your inbox—but the real danger can creep in silently over months or years. History is full of examples where personal data or records got collected for one benign purpose and ended up being used for something far worse down the line.

The Real Problem

1. ==**Data is power.**==
    Our computers and the internet started out as tools for solving math problems and sending emails. Now they shape shopping habits, political opinions, and even personal relationships.

2. ==**We can’t foresee all the ways data might be used.**==
    ==Technology evolves rapidly. Today’s harmless info can become tomorrow’s goldmine for manipulation—by corporations, adversaries, or rogue third parties.==

3. **Transparency matters.**
    Hiding data practices or collecting info without consent leads to deceptive power imbalances. If you don’t know _who_ is doing _what_ with your data, how can you protect yourself or our members?

Bottom Line

- ==**Privacy is about choice**: If you don’t control who has your data, _they_ control your narrative (and possibly your future).==
- **Small leaks accumulate**: Each seemingly inconsequential tidbit can be pieced together into a revealing (and exploitable) big picture.
- ==**Your values matter**: Data can be sold, shared, or analyzed in ways that directly oppose your personal or organizational values—even if you never see it happening.==

==When you hear “I’ve got nothing to hide,” remember it’s not about hiding shady activities. It’s about preventing power imbalances and ensuring our data isn’t used against us—or those we serve—in ways we can’t predict.==

==So yes, you _do_ have something to hide. And that “something” is your ability to shape your own story, your own decisions, and your own future—without unwanted interference.==

_Stay informed, stay empowered, and let’s keep our data where it belongs: under our control._

#### D1) Workspace

Why the "I've got nothing to hide" argument doesn't fly anymore.
Why Data Privacy matters.

- What is Data Privacy?
	- Data privacy generally means the ability of a person to determine for themselves when, how, and to what extent personal information about them is shared with or communicated to others.
	- Privacy assumes that knowledge about you can be used to "hurt" you or your goals

- Is someone right just because they are persuasive?

- Can data about you be used to hurt your goals?
- Can data be used to hurt our members' goals?
- Can data be used to hurt our mission to our members?

- The answer is all yes.

- Nowadays,
	- Data is power
	- We made computers at first to solve hard math problems, but computers and the communication of them (the internet) have evolved to encompass every aspect of our lives
	- As they become an integral part of our lives, they eat up tons of data about people
- Data privacy is important, because the data about us (namely our interests) can be used to harm those interests
	- It's not about a country or the NSA spying on you or looking at what you're buying on Amazon.  It's about the less obvious things that happen over long periods of time.

- The ways of upholding data privacy personally, as a business, and for our members affect way more than just the case of persuasive technologies and big data analytics.  However, the main goal of data privacy is to prevent a system where those with data processing power can control those without it.  It's about transparency and being less deceptive.

- If your data can be used to further the interests of those who's interests conflict with your own, then you should care.  It is true that someone who has enough data about you can use it in a way that doesn't align with your values.  Therefore, data privacy matters to everyone.

- Let me phrase it another way...
	- Having control over who gets data about you matters, because it can be used in ways that hurt your goals.  ==The problem with data privacy is that it's hard to quantify the damage as it happens in little ways over time==.
- ==We can't anticipate what others can do with data about us.  They might not even directly use it to exploit you.  They may use it to hurt other people or for their business which goes against your values.  We all won't agree on what's right or what's wrong.  No matter what, whoever your "enemy" is, they can wield data about you in ways that hurt your goals.==

- ==One issue that prevents privacy from becoming a priority is that the effects are harder to see in the short term especially as you scale it out to a nation rather than just a person.==

- ==we are wired for the short term and immediacy.==  People are to a point better at defending against things like scams and sharing super sensitive data because the damage is done in a very noticeable short-term way.  We are pretty aware when we are being exploited in the short term or in danger with some immediate threat.  But data privacy isn't like the bear in the woods.  It's more like a creeping fog of poisonous gas.  Overtime as we put our data into more and more services and apps, this data is built up to make a detailed profile about each and every one of us.  That in and of itself is not a danger but rather it can be used in ways that are dangerous.  That's a more personal level you could see data about you being used to take up your time by being on your screen more with advertisements, maybe cluttering your email or text messages with spam, and then we get to the issues that are larger scale.  For instance maybe a business has enough data to market to certain individuals and become successful.  However that business used your data along with thousands of other people to further their business schools.  The issue being that those goals might very well be against your own or even malicious in the grand scheme of things.  And then we get up to the nation state risks like countries doing espionage and things like that.  At that level we're looking at some risks that I wouldn't prioritize but are still a problem.  I'm sure many countries are using this data to think about how they can change our interests right or the things that we care about.  The nation state goal here is espionage and eroding interests of other countries slowly.
- at the level that we care more about the problem is that we have trouble weighing convenience against the potential harms especially when it's with technology because it's so abstract
- generally with these complex industries that have a lot of deep knowledge about that particular niche and come with high risks for most people, usually the government does something about it or they put systems in place for us to make those risk assessments and decisions ourselves
- in the case of data privacy some countries have already done that however there is definitely a lot of pushback the past few years in the United States
- All this definitely becomes more prevalent with AI too because AI is really good at maximizing some goal.  In the case of marketing or advertisements that's getting people to buy things, and we don't know what the side effects are of the AI trying to reach those goals on a large scale.  But also maybe it's to change people's interests or goals or preferences.  No matter what the goal is of using AI, issue now becomes that some business or entity has way more power than most people and they gain that power when they get more data about people.  For AI more data is more power and that's the risk for everyone and businesses

==Societal advancements—think industrialization or even the internet—inevitably introduce new risks. When those risks exploit people, we create laws and frameworks to protect individuals and ensure fairness.== Data privacy should follow the same principle: it's not about rejecting technology but about making it safe and equitable.

==For example, labor laws prevent companies from exploiting workers; environmental regulations protect shared natural resources. Why wouldn’t we apply similar protections to our personal data, which is just as valuable?==

Here’s a blog addition:

**Bullet Point: Societal Progress Requires Guardrails**

- ==Every major technological leap has come with new protections—data privacy should be no exception.==
- Exploiting personal data without consent is akin to exploiting labor or resources.
- Regulations and transparency ensure innovation benefits _everyone_, not just powerful companies.

### Day 2) Intranet Post

What we do with member/customer data
Our responsibilities
How we use and protect member/customer data.

People who wield data in ways that conflict with our mission:
- hackers (sell the data)(hold the data hostage - extortion)
- fraudsters (steal member resources or money)
- insider threats (accidentally sharing sensitive data online)
- other businesses (use the data in ways that conflict with our mission)

### Day 3) Webinar & Presentation

- Welcome and Introduction (5 min)
	- Opening Remarks:
		- Welcome Attendees
		- Share session objectives
		- Q&A engagement expectation
	- Brief introduction of the expert
- Expert Keynote
	- Ben shares insights on: (15 min)
		- The importance of data privacy
		- Common Threats to Data Privacy
		- Key Practices for Individuals and Organizations
		- Organizational Data Privacy Measures
- Live Q&A
	- Pre-Submitted Questions (20 min)
		- Address questions collected from attendees during registration
	- Open-Floor Questions (20 min)
- Wrap-up and Takeaways (5 min)
	- Ben shares 3-5 key actionable items
	- Thank Ben and participants
	- Provides information about Data Privacy resources ()

#### Presentation:

- Define Data Privacy and the role it plays in today’s digital word
- Why it matters and how it impacts individuals, organizations, society
- Know what information you are sharing and how it is collected/obtained
- How to understand privacy policies/laws
- Why it matters. Discuss growing risk (breaches, misuse)

- Let's do a poll
	- If you had to put yourself in a group...which one:
		- I've got nothing to hide
		- I would like to live off-grid with no internet access, the paranoid - "I would never use Google"
		- I don't like the idea of big companies having my data, but I don't know why

### Day 4)

Tools & Tips - 3 steps to better personal privacy

-

### Day 5)

Kahoot + Blog

- Ideas for blog topic:
	- laws, upcoming laws…
	- Resources, experts to follow