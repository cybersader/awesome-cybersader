...
var _TreeNode = class {
  constructor(value) {
    ...
    // Cache for the number of descendant nodes.
  ...
  newChild(value) {
    const childNode = new _TreeNode(value);
    this._children.push(childNode);
    this._descendantNodeCount = null;
    return childNode;
  }
  findChildValue(value) {
  ...
  hasChildValue(value) {
    return this.children.some((ch) => {
      return ch.value === value;
    });
  }
  forEach(callback, order = "preorder") {
  ...
  *postorderTraversal() {
  ...
  computeLeafCount() {
    ...
    return this._children.reduce((acc, child) => acc + child.leafCount, 0);
  }
  get descendantNodeCount() {
    if (this._descendantNodeCount === null) {
      this._descendantNodeCount = this.computeDescendantNodeCount();
    }
    return this._descendantNodeCount;
  }
  computeDescendantNodeCount() {
...
var DataService = class {
  ...
  get glyphFilePathNodeMap() {
    if (this._glyphFilePathNodeMap === void 0) {
  ...
  get vaultFileRecords() {
    if (!this._vaultFileRecords || this._vaultFileRecords.length === 0) {
      return this.refresh();
    }
    return this._vaultFileRecords;
  }
  superordinateChains(filePath, relationshipDefinitions, isExpandSelfSubtree = false, selfSubtreeDepthLimit = null) {
    let startFileNode = this.getFileNode(filePath);
    let filePathNodeMap = /* @__PURE__ */ new Map();
    let startChildren = [];
    ...
    if (startChildren.length > 0) {
      superordinateWalkData.leafNodes.forEach((leafNode) => {
        leafNode.children = [...startChildren];
      });
    }
    return results;
  }
  subordinateSubtrees(filePath, relationshipDefinitions, limitDepth = null, filePathNodeMap) {
    if (!filePathNodeMap) {
      filePathNodeMap = /* @__PURE__ */ new Map();
    }
    let startFileNode = filePathNodeMap.get(filePath) || this.getFileNode(filePath);
    filePathNodeMap.set(filePath, startFileNode);
...
var _FileNode = class {
  constructor(filePath, dataService, displayText) {
    this.relationships = {};
    this._cache = {};
  ...
  createNew(filePath, displayText = null) {
  ...
  registerRelationship(filePath, relationshipData) {
  ...
  readDesignatedPropertyPaths(propertyName, pathAliases) {
    ...
    return propertyValue.filter((item) => item == null ? void 0 : item.path).map((item) => {
  ...
  readInlinkedRelationshipPropertyPaths(propertyName) {
  ...
  parsePropertyLinkedPaths(primaryRelationshipKey, inlinkedRelationshipKey, filePathNodeMap, pathAliases, isInvertLinkPolarity = false) {
    ...
    if (inlinkedRelationshipKey) {
      let inlinkedPaths = this.readInlinkedRelationshipPropertyPaths(inlinkedRelationshipKey);
      this._processPropertyLinkResults(
    ...
    return [...new Set(linkedNotesystemPaths)];
  }
  superordinateChains(label, relationshipDefinitions, limitHeight, filePathNodeMap) {
    if (this._superordinateChains === void 0) {
      let results = {
        rootNodes: [],
        leafNodes: []
      };
      ...
      relationshipDefinitions.forEach((relationshipDefinition) => {
        ...
        linkedNotesystemPaths.forEach((propertyLinkedPath) => {
          if (propertyLinkedPath === this.filePath) {
          } else {
            let connectedSuperordinateChains;
            let linkedNoteSystemNode = filePathNodeMap.get(propertyLinkedPath);
            if (linkedNoteSystemNode === void 0) {
              linkedNoteSystemNode = this.createNew(propertyLinkedPath, pathAliases[propertyLinkedPath] ? pathAliases[propertyLinkedPath][0] : void 0);
            ...
            connectedSuperordinateChains.rootNodes.forEach((chain) => {
              results.rootNodes.push(chain);
            });
            connectedSuperordinateChains.leafNodes.forEach((chain) => {
      ...
      results.rootNodes = [...new Set(results.rootNodes)];
      results.leafNodes = [...new Set(results.leafNodes)];
      this._superordinateChains = results;
    }
    return this._superordinateChains;
  }
  subordinateSubtrees(label, relationshipDefinitions, limitDepth = null, filePathNodeMap) {
  ...
  readGlyphs(propertyNames, limitDepth = null) {
    let nodeGlyphs = [];
    if (limitDepth != null && limitDepth < 0) {
      return nodeGlyphs;
    }
    ...
    propertyNames.forEach((propertyKey) => {
      ...
      (((_b = (_a = this.fileData) == null ? void 0 : _a.frontMatterCache) == null ? void 0 : _b[propertyKey]) || []).forEach((fieldValue) => {
        if (fieldValue.path !== void 0) {
          ...
          if (referencedGlyphNode === void 0) {
            referencedGlyphNode = this.createNew(referencedGlyphPath, void 0);
  ...
  inlinkedFilePaths() {
    var _a, _b;
    const inlinks = ((_b = (_a = this.fileData) == null ? void 0 : _a.file) == null ? void 0 : _b.inlinks) || [];
    return inlinks.map((link) => link.path);
  }
  backlinkedFileNodes() {
    var _a, _b, _c;
  ...
  sort_key(other) {
    ...
    const b = other.indexEntryText || other.fileBaseName || "";
    const result = a.localeCompare(b);
    return result;
  }
};
var FileNode = _FileNode;
__decorateClass([
  Cacheable("label", "relationshipDefinitions", "limitHeight")
], FileNode.prototype, "superordinateChains", 1);
__decorateClass([
  Cacheable("label", "relationshipDefinitions")
], FileNode.prototype, "subordinateSubtrees", 1);
__decorateClass([
  Cacheable("label", "relationshipDefinitions")
], FileNode.prototype, "coordinateSubtrees", 1);
__decorateClass([
...
${value}`);
  ...
  menu.addItem(
    (item) => item.setTitle("Copy as quoted internal link").setIcon("clipboard-copy").onClick(() => _copyToClipboard(`"[[${internalPath}]]"`))
  );
  menu.addItem(
    (item) => item.setTitle("Copy as property item link").setIcon("clipboard-copy").onClick(() => _copyToClipboard(`  - "[[${internalPath}]]"`))
  );
  menu.addItem(
...
var InputNumberComponent = class {
  constructor(containerEl, minValue, maxValue, initialValue, onChangeCallback, label) {
    this.isMapMinValueToNull = true;
  ...
  render() {
    ...
    this.inputEl = inputAndButtonsContainer.appendChild(document.createElement("input"));
...
var BearingsView = class extends import_obsidian9.ItemView {
  constructor(leaf, plugin, configuration, dataService) {
    ...
    this.plugin.registerHoverLinkSource(
      ...
      {
        defaultMod: true,
        /* require ctrl key trigger */
        display: DISPLAY_TEXT
      }
    );
    this.onActiveLeafChange = this.onActiveLeafChange.bind(this);
  }
  computeActiveFilePath() {
    var _a;
    let activeFilePath = ((_a = this.plugin.app.workspace.getActiveFile()) == null ? void 0 : _a.path) || "";
...
var NavigationView = class extends NavigationBase {
  ...
  async render(targetFilePath, options = {}) {
    if (!options.isForced) {
      if (this.isPinned) {
        return true;
      }
...
var NavigationViewFrame = class extends NavigationBase {
  constructor(navigationContext, root, title) {
    super(
      navigationContext,
      root
    );
  ...
  _validateDepthLimitSetting(ival) {
    if (ival === "") {
      return null;
    }
  ...
  async render() {
    ...
    results.treeNodes.sort((a, b) => a.value.sort_key(b.value)).forEach((entryData) => {
      ...
      viewEntry.setupCallbackFn = (entryFrame, entryData2) => this.setLocalOptions(entryFrame, entryData2);
      viewEntry.render(
        entryData,
        results.parentFileNode || void 0,
        this.autoexpansionDepthLimit
      );
    });
  }
};
var SuperordinateRelationshipsAscendersViewFrame = class extends NavigationViewFrame {
  constructor(navigationContext, root, title) {
    super(navigationContext, root, title);
    this.isFocalFileExpandedOnce = false;
    this.nFocalFileSeen = 0;
    this.nFocalFileSeen = 0;
  }
  getDefaultDiscoveryDepthLimit() {
...
var CoordinateRelationshipsViewFrame = class extends NavigationViewFrame {
  getDefaultDiscoveryDepthLimit() {
...
var ParallelRelationshipsViewFrame = class extends NavigationViewFrame {
  getDefaultDiscoveryDepthLimit() {
    return this._validateDepthLimitSetting(this._context.configuration.options.discoveryDepthLimitSecondary);
  }
  setLocalOptions(entryFrame, entryData) {
  ...
  generateResults() {
    ...
    let fileNodePathMap = /* @__PURE__ */ new Map();
    ...
    currentFileSuperordinates.forEach((superordinate) => {
      treeNodes.push(
...
var BacklinkedRelationshipsViewFrame = class extends NavigationViewFrame {
  ...
  generateResults() {
    let currentFileNode = this._context.dataService.getFileNode(this.focalFilePath);
    let backlinkedNodes = currentFileNode.backlinkedFileNodes().map((fileNode) => {
...
var NavigationEntryFrame = class extends NavigationBase {
  constructor(navigationContext, root, parentViewFrame, isPostFocalFile) {
    super(
      navigationContext,
      root
    );
  ...
  //     // `;
  // }
  get isHighlightFocalFile() {
    var _a;
    return (_a = this.options.isHighlightFocalFile) != null ? _a : true;
  }
  get isDefaultOpenFocalFile() {
    var _a;
    return (_a = this.options.isDefaultOpenFocalFile) != null ? _a : true;
  }
  get isDefaultOpen() {
    var _a;
    return (_a = this.options.isDefaultOpen) != null ? _a : this.parentViewFrame.isDefaultOpen;
  }
  async render(entryData, parentFileNode = null, defaultOpenDepthLimit) {
    var _a;
    if (this.setupCallbackFn !== void 0) {
      this.setupCallbackFn(
        this,
        entryData
      );
    }
    let subtreeDefaultOpenDepthLimit = defaultOpenDepthLimit === null ? null : defaultOpenDepthLimit - 1;
    ...
    let setIndicator = () => {
      if (entryData.children.length > 0) {
        ...
        if (this.isOpen) {
          this.elements.entryNodeToggleContainer.setText("\u{1F837}");
          Object.values(this.elements).forEach((el) => {
            el.classList.remove("bearings-node-closed");
            el.classList.add("bearings-node-opened");
          });
        } else {
          this.elements.entryNodeToggleContainer.setText("\u{1F836}");
          Object.values(this.elements).forEach((el) => {
    ...
    [...entryData.children].sort((a, b) => a.value.sort_key(b.value)).forEach((childNode) => {
  ...
  async onload() {
    ...
    this.registerEvent(
      this.app.workspace.on("editor-menu", (menu, editor, view) => {
        var _a;
        if (menu && ((_a = view == null ? void 0 : view.file) == null ? void 0 : _a.path)) {
