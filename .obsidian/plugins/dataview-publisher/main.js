...
var require_lib = __commonJS({
  "node_modules/obsidian-dataview/lib/index.js"(exports) {
    ...
    parsimmon_umd_min.exports.alt(parsimmon_umd_min.exports.regex(new RegExp(emojiRegex(), "")), parsimmon_umd_min.exports.regex(/[0-9\p{Letter}_-]+/u).map((str) => str.toLocaleLowerCase()), parsimmon_umd_min.exports.whitespace.map((_) => "-"), parsimmon_umd_min.exports.any.map((_) => "")).many().map((result2) => result2.join(""));
    var HEADER_CANONICALIZER = parsimmon_umd_min.exports.alt(parsimmon_umd_min.exports.regex(new RegExp(emojiRegex(), "")), parsimmon_umd_min.exports.regex(/[0-9\p{Letter}_-]+/u), parsimmon_umd_min.exports.whitespace.map((_) => " "), parsimmon_umd_min.exports.any.map((_) => " ")).many().map((result2) => {
...
var SettingTab = class extends import_obsidian.PluginSettingTab {
  ...
  display() {
    ...
    containerEl.empty();
    new import_obsidian.Setting(containerEl).setName("Source").setDesc("Dataview source to search for target files.").addTextArea(
      (text) => text.setPlaceholder("#dataview-publisher").setValue(this.plugin.settings.source).onChange(async (value) => {
    ...
    new import_obsidian.Setting(containerEl).setName("Update on save").setDesc(
...
// src/errors.ts
var _StartBlockNotFoundError = class extends Error {
};
var StartBlockNotFoundError = _StartBlockNotFoundError;
(() => {
  _StartBlockNotFoundError.prototype.name = "StartBlockNotFoundError";
})();
var _EndBlockNotFoundError = class extends Error {
};
var EndBlockNotFoundError = _EndBlockNotFoundError;
(() => {
  _EndBlockNotFoundError.prototype.name = "EndBlockNotFoundError";
})();

// src/dataview-publisher.ts
var START_BLOCK_REGEX = /\s*%%\s*DATAVIEW_PUBLISHER:\s*start\s*[\s\S]*?%%\s*/;
var END_BLOCK_REGEX = /\s*%%\s*DATAVIEW_PUBLISHER:\s*end\s*%%\s*/;
var BLOCK_REGEX = new RegExp(
  START_BLOCK_REGEX.source + /(?<output>[\s\S]*?)/.source + END_BLOCK_REGEX.source,
  "gm"
);
async function createReplacerFromContent(content, dv2, tfile2) {
  const blocks = extractBlocks(content);
  const updatedBlocks = await updateBlocks(blocks, dv2, tfile2);
  const replacer = createReplacer(blocks, updatedBlocks);
  return replacer;
}
function createReplacer(existing, updated) {
  return existing.map((e, i) => ({
    searchValue: e.content,
    replaceValue: updated[i].content
  }));
}
async function updateBlocks(blocks, dv2, tfile2) {
  return await Promise.all(
    blocks.map((block2) => {
      return updateBlock(block2, dv2, tfile2);
    })
  );
}
async function updateBlock(block2, dv2, tfile2) {
  const executionResult = await executeBlock(block2, dv2, tfile2);
  return {
    ...block2,
...
function parseBlock(block2) {
  ...
  return {
    ...
    query,
    output,
    endBlock: extractEndBlock(block2)
  };
}
function extractStartBlock(text) {
...
function composeBlockContent(blocks) {
  const { startBlock, output, endBlock } = blocks;
  return startBlock + "\n\n" + output + "\n\n" + endBlock;
}

// src/dataview-utils.ts
var import_obsidian_dataview = __toESM(require_lib());
function getDataviewAPI(app) {
  const api = (0, import_obsidian_dataview.getAPI)(app);
  if (!api) {
...
var Operator = class {
  ...
  async updateActiveFile(editor) {
    const cursor = editor.getCursor();
    const content = editor.getValue();
    const tfile2 = this.getActiveTFile();
  ...
  updateFromSource(source) {
    ...
    targetTfiles.forEach(async (tfile2) => {
      await this.updateDataviewPublisherOutput(tfile2);
    });
  }
  retrievePathsFromSource(source) {
    var _a;
    const paths = (_a = this.dv.pagePaths(source).map((path) => this.dv.io.normalize(path)).array()) != null ? _a : [];
    return paths;
  }
  retrieveTfilesFromSource(source) {
    const paths = this.retrievePathsFromSource(source);
    const tfiles = paths.map((path) => this.app.vault.getFileByPath(path)).filter((tfile2) => tfile2 !== null);
    return tfiles;
  }
...
// src/main.ts
var Main = class extends import_obsidian3.Plugin {
  constructor() {
    super(...arguments);
    __publicField(this, "settings");
    __publicField(this, "originalSaveCallback");
  }
  async onload() {
    await this.loadSettings();
    this.addSettingTab(new SettingTab(this.app, this));
    createCommands(this.app, this.settings).forEach((command) => {
    ...
    if (typeof saveCommandDefinition.callback === "function") {
      saveCommandDefinition.callback = () => {
        ...
        if (this.settings.updateOnSave && editor) {
          try {
            const operator = new Operator(this.app);
