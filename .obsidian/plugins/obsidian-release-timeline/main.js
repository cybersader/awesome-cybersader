  ...
  "node_modules/obsidian-dataview/lib/index.js"(exports) {
    ...
    var DateTime2 = class {
      ...
      toRelativeCalendar(options = {}) {
        if (!this.isValid)
          return null;
        return diffRelative(options.base || DateTime2.fromObject({}, { zone: this.zone }), this, __spreadProps(__spreadValues({}, options), {
          numeric: "auto",
    ...
    var isPluginEnabled5 = (app) => app.plugins.enabledPlugins.has("dataview");
    ...
    exports.isPluginEnabled = isPluginEnabled5;
...
function createRowSeparatorYearMonth(type) {
...
function createRowSeparatorWeek(type) {
  const newTdSeparator1 = createEl("td", { cls: "td-separator" });
...
function parseQuerySortOrder(content, plugin) {
    ...
    timeline.forEach((item) => {
      ...
      if (value.length == 1) {
        ...
        for (let i = 1; i < value.length; i++) {
          const rowItem2 = createRowItem({ fileName: value[i][0], fileAlias: value[i][1], cls: "td-next" });
...
var MonthTimeline = class {
  constructor(plugin) {
    this.plugin = plugin;
  }
  renderTimelineMonth(content) {
    return __async(this, null, function* () {
      const dv = (0, import_obsidian_dataview2.getAPI)();
      if (typeof dv == "undefined") {
        return createErrorMsg("Dataview is not installed. The Release Timeline plugin requires Dataview to properly function.");
      }
      let dvResults;
...
var WeekTimeline = class {
  constructor(plugin) {
    this.plugin = plugin;
  }
  renderTimelineWeek(content) {
    return __async(this, null, function* () {
      const dv = (0, import_obsidian_dataview3.getAPI)();
      if (typeof dv == "undefined") {
        return createErrorMsg("Dataview is not installed. The Release Timeline plugin requires Dataview to properly function.");
      }
      let dvResults;
  ...
  insertEmptyWeeksCollapsedNo(contentData) {
    ...
    const minWeek = (0, import_obsidian3.moment)(existingWeeks[0]);
    const maxWeek = (0, import_obsidian3.moment)(existingWeeks[existingWeeks.length - 1]);
    for (let week = minWeek; week.isSameOrBefore(maxWeek); week.add(1, "weeks")) {
      ...
      if (!existingWeeks.includes(weekFormatted)) {
        const newYear = (0, import_obsidian3.moment)(week).add(3, "days").format("Y");
        const newMonth = (0, import_obsidian3.moment)(week).add(3, "days").format("Y-MM");
        const newWeek = weekFormatted;
        const newWeekStart = week.format("GGGG-MM-DD");
        const newWeekEnd = (0, import_obsidian3.moment)(week).add(6, "days").format("GGGG-MM-DD");
        const newWeekObject = {
          year: newYear,
          month: newMonth,
          week: newWeek,
  ...
  collapseEmptyWeeks(fullWeekTimelineData) {
    ...
    for (let month = (0, import_obsidian3.moment)(minMonth); month.isSameOrBefore((0, import_obsidian3.moment)(maxMonth)); month.add(1, "months")) {
      const monthFormatted = month.format("Y-MM");
  ...
  renderMonthsInYear(timelineDataFilteredByYear) {
    ...
    while (timelineDataFilteredByYear.length != 0) {
      ...
      const currMonthText = (0, import_obsidian3.moment)(timelineDataFilteredByYear[0].month).format("MMM");
  ...
  setWeekFormatting(weekData) {
    ...
    const weekNb = (0, import_obsidian3.moment)(weekData.weekStart).format("WW");
    const weekStart = (0, import_obsidian3.moment)(weekData.weekStart).format("DD");
    const weekEnd = (0, import_obsidian3.moment)(weekData.weekEnd).format("DD");
    const monthNbStart = (0, import_obsidian3.moment)(weekData.weekStart).format("MM");
    const monthNbEnd = (0, import_obsidian3.moment)(weekData.weekEnd).format("MM");
    let weekText;
    if (weekData.collapsed == true) {
      weekText = (0, import_obsidian3.moment)(weekData.month).format("MMM");
...
var SampleSettingTab = class extends import_obsidian4.PluginSettingTab {
  ...
  display() {
    ...
    new import_obsidian4.Setting(containerEl).setName("Default sort order").setDesc("Used if sort order is not provided in a query").addDropdown((dropdown) => {
    ...
    new import_obsidian4.Setting(containerEl).setName("Bullet points").setDesc("Improves readability for time periods with multiple entries").addToggle((toggle) => {
    ...
    new import_obsidian4.Setting(containerEl).setName("Collapse empty years").setDesc("Consecutive empty years will be collapsed into one range (2000-2018)").addToggle((toggle) => {
    ...
    new import_obsidian4.Setting(containerEl).setName("Minimum number of years to be collapsed").addText(
      (text) => text.setPlaceholder("2").setValue(this.plugin.settings.collapseLimit).onChange((value) => __async(this, null, function* () {
        this.plugin.settings.collapseLimit = value;
        yield this.plugin.saveSettings();
      }))
    );
    containerEl.createEl("h3", { text: "Week timeline settings" });
    new import_obsidian4.Setting(containerEl).setName("Collapse empty months").setDesc("Weeks will not be displayed for months without actual data").addToggle((toggle) => {
    ...
    new import_obsidian4.Setting(containerEl).setName("Week formatting").addDropdown((dropdown) => {
...
// main.ts
var ReleaseTimeline = class extends import_obsidian5.Plugin {
  constructor() {
    super(...arguments);
    this.YearTimelineFunctions = new YearTimeline(this);
    this.MonthTimelineFunctions = new MonthTimeline(this);
    this.WeekTimelineFunctions = new WeekTimeline(this);
  }
  onload() {
    return __async(this, null, function* () {
      this.app.workspace.onLayoutReady(() => {
        const isDataviewInstalled = !!(0, import_obsidian_dataview4.getAPI)();
        if (!isDataviewInstalled) {
          new import_obsidian5.Notice("The Release Timeline plugin requires Dataview to properly function.");
        }
      });
      ...
      this.registerMarkdownCodeBlockProcessor("release-timeline", (content, el, ctx) => __async(this, null, function* () {
        let timelineTable = yield this.YearTimelineFunctions.renderTimeline(content);
      ...
      this.registerMarkdownCodeBlockProcessor("release-timeline-month", (content, el, ctx) => __async(this, null, function* () {
        let timelineTable = yield this.MonthTimelineFunctions.renderTimelineMonth(content);
        el.appendChild(timelineTable);
        let bulletOption = this.settings.bulletPoints;
        let matches = el.querySelectorAll(".td-first, .td-next");
        matches.forEach(function(match) {
      ...
      this.registerMarkdownCodeBlockProcessor("release-timeline-week", (content, el, ctx) => __async(this, null, function* () {
        let timelineTable = yield this.WeekTimelineFunctions.renderTimelineWeek(content);
        el.appendChild(timelineTable);
        let bulletOption = this.settings.bulletPoints;
