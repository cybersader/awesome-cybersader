...
var TagTacticianSettingTab = class extends import_obsidian3.PluginSettingTab {
  ...
  display() {
    ...
    new import_obsidian3.Setting(containerEl).setName("Default navigation sorting").setDesc("Sort alphabetically or by notes with the tag.").addDropdown((dropdown) => {
      dropdown.addOption("alphabetically-descending", "Alphabetically").addOption("file-count-descending", "By note count").setValue(this.plugin.settings.nbtDefaultSort).onChange(async (value) => {
...
var NavByTagView = class extends import_obsidian6.ItemView {
  constructor(leaf, plugin) {
    super(leaf);
   ...
   */
  filterHierarchy(hierarchy, filterQuery) {
    if (!filterQuery)
      return hierarchy;
    const result = {};
    for (const [key, { files, children }] of Object.entries(hierarchy)) {
      ...
      const matchingFiles = new Set(
        [...files].filter(
          (file) => file.basename.toLowerCase().includes(filterQuery)
        )
      );
      ...
      if (tagNameMatches || matchingFiles.size > 0 || Object.keys(filteredChildren).length > 0) {
        result[key] = {
          files: /* @__PURE__ */ new Set([...matchingFiles]),
          children: filteredChildren
        };
        if (tagNameMatches && matchingFiles.size === 0 && files.size > 0) {
          result[key].files = /* @__PURE__ */ new Set([...files]);
        }
      }
    }
    return result;
   ...
   */
  renderTagGroup(container, group, path = []) {
...
// main.ts
var TagTacticianPlugin = class extends import_obsidian7.Plugin {
  constructor() {
    super(...arguments);
    ...
    menu.addItem((item) => {
      item.setTitle("Edit tags (frontmatter)").setIcon("hashtag").onClick(async () => {
        ...
        new EditTagsModal(this.app, allItems, async (updates) => {
          ...
          new import_obsidian7.Notice(`Updated frontmatter tags in ${modifiedCount} file(s).`);
        }).open();
      });
    });
  }
...
function expandFolders(selection) {
  ...
  for (const item of selection) {
    if (item instanceof import_obsidian7.TFolder) {
      import_obsidian7.Vault.recurseChildren(item, (child) => {
