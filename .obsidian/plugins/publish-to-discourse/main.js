...
function setLocale(locale) {
  ...
  if (localeMap[normalizedLocale]) {
    ...
    import_obsidian.moment.locale(normalizedLocale);
  }
}
function t(key) {
...
var DiscourseSyncSettingsTab = class extends import_obsidian2.PluginSettingTab {
  ...
  display() {
    ...
    containerEl.empty();
    new import_obsidian2.Setting(containerEl).setName(t("FORUM_URL")).setDesc(t("FORUM_URL_DESC")).addText(
      (text) => text.setPlaceholder("https://forum.example.com").setValue(this.plugin.settings.baseUrl).onChange(async (value) => {
        this.plugin.settings.baseUrl = value;
        await this.plugin.saveSettings();
      })
    );
    new import_obsidian2.Setting(containerEl).setName(t("API_KEY")).setDesc(t("API_KEY_DESC")).addText(
      (text) => text.setPlaceholder("api_key").setValue(this.plugin.settings.apiKey).onChange(async (value) => {
        this.plugin.settings.apiKey = value;
        await this.plugin.saveSettings();
      })
    );
    new import_obsidian2.Setting(containerEl).setName(t("USERNAME")).setDesc(t("USERNAME_DESC")).addText(
      (text) => text.setPlaceholder("username").setValue(this.plugin.settings.disUser).onChange(async (value) => {
...
function findTagObject(value, tagName, tags) {
  ...
  if (tagName) {
    ...
    const tagObj = (_a = match.find((t2) => !t2.format)) != null ? _a : match[0];
    if (!tagObj)
...
function getTagObject(tags, item) {
  ...
  if (item.tag) {
    ...
    if (match.length > 0)
      return (_a = match.find((t2) => t2.format === item.format)) != null ? _a : match[0];
  }
  let tagObj = void 0;
  let obj;
  if (isScalar(item)) {
...
async function expandEmbeds(app, file, stack = [], subpath) {
  const sp = subpath != null ? subpath : "<entireFile>";
...
function sliceSubpathContent(app, tfile, fileContent, subpath) {
  var _a;
  ...
  if (subpath.startsWith("^")) {
    ...
    const block = (_a = fileCache.blocks) == null ? void 0 : _a[blockId];
    if (!block) {
      return "";
    }
...
function sliceHeading(content, fileCache, headingName) {
...
// src/main.ts
var DiscourseSyncPlugin = class extends import_obsidian3.Plugin {
  async onload() {
    setLocale(import_obsidian3.moment.locale());
    this.registerEvent(
  ...
  async uploadImages(imageReferences) {
    var _a;
    const imageUrls = [];
    for (const ref of imageReferences) {
      const filePath = (_a = this.app.metadataCache.getFirstLinkpathDest(ref, this.activeFile.name)) == null ? void 0 : _a.path;
      if (filePath) {
        ...
        if (abstractFile instanceof import_obsidian3.TFile) {
          try {
            ...
            const imgForm = `${sBoundary}Content-Disposition: form-data; name="file"; filename="${abstractFile.name}"\r
Content-Type: image/${abstractFile.extension}\r
\r
`;
            let body = "";
            body += `\r
${sBoundary}Content-Disposition: form-data; name="type"\r
\r
composer\r
`;
            body += `${sBoundary}Content-Disposition: form-data; name="synchronous"\r
\r
true\r
`;
            const eBoundary = "\r\n--" + boundary + "--\r\n";
  ...
  async postTopic() {
    ...
    const postId = frontMatter == null ? void 0 : frontMatter.discourse_post_id;
    ...
    const body = JSON.stringify(isUpdate ? {
      ...
      title: (frontMatter == null ? void 0 : frontMatter.title) ? frontMatter == null ? void 0 : frontMatter.title : this.activeFile.name,
      raw: content,
  ...
  async updateFrontMatter(postId, topicId) {
    try {
...
${content}`;
  ...
  async fetchCategories() {
    ...
    try {
      ...
      const allCategories = categories.flatMap((category) => {
        var _a;
        const subcategories = ((_a = category.subcategory_list) == null ? void 0 : _a.map((sub) => ({
          id: sub.id,
          name: sub.name
        }))) || [];
        return [
    ...
    const syncDiscourse = (item) => {
      ...
      item.onClick(async () => {
        ...
        this.activeFile = {
          name: file.basename,
          content,
          postId: fm == null ? void 0 : fm.discourse_post_id
        };
  ...
  async syncToDiscourse() {
    await this.openCategoryModal();
  }
  async openInDiscourse() {
    const activeFile = this.app.workspace.getActiveFile();
    if (!activeFile) {
    ...
    const content = await this.app.vault.read(activeFile);
    const fm = this.getFrontMatter(content);
    const discourseUrl = fm == null ? void 0 : fm.discourse_url;
    const topicId = fm == null ? void 0 : fm.discourse_topic_id;
...
var NotifyUser = class extends import_obsidian3.Modal {
...
var SelectCategoryModal = class extends import_obsidian3.Modal {
  constructor(app, plugin, categories, tags) {
    super(app);
    this.canCreateTags = false;
    this.plugin = plugin;
  ...
  onOpen() {
    ...
    const isUpdate = this.plugin.activeFile.postId !== void 0;
    ...
    tagInput.oninput = () => {
      ...
      if (value) {
        const matches = this.tags.filter(
          (tag) => tag.name.toLowerCase().includes(value) && !selectedTags.has(tag.name)
        ).slice(0, 10);
        if (matches.length > 0) {
          const inputRect = tagInput.getBoundingClientRect();
          const modalRect = this.modalEl.getBoundingClientRect();
          const maxWidth = modalRect.right - inputRect.left - 24;
    ...
    tagInput.onkeydown = (e) => {
      if (e.key === "Enter" && tagInput.value) {
        ...
        if (value && !selectedTags.has(value)) {
          const existingTag = this.tags.find((t2) => t2.name.toLowerCase() === value.toLowerCase());
          if (existingTag) {
