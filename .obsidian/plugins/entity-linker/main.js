...
__export(main_exports, {
  EntitySuggestionModal: () => EntitySuggestionModal,
  default: () => EntityLinker
});
...
var FileInputSuggest = class extends import_obsidian.AbstractInputSuggest {
  ...
  getFolders(folder) {
    ...
    const filteredChildren = f.children.filter((val) => val instanceof import_obsidian.TFolder);
...
// node_modules/obsidian-file-suggestion-component/dist/index.js
var FileSuggestionComponent = class extends import_obsidian2.TextComponent {
  /**
...
var EntitySuggestionModal = class extends import_obsidian3.SuggestModal {
  constructor(app, search_term, polite_email, onSubmit) {
    super(app);
  ...
  async getSuggestionsImpl(query) {
  ...
  async getWikiDataFromSearchTerm(search_term) {
  ...
  async generatePropertiesFromEntity(entity) {
  ...
  async onChooseSuggestion(entity, evt) {
    const entity_props = await this.generatePropertiesFromEntity(entity);
    this.onSubmit(entity_props);
  }
};
var EntityLinker = class extends import_obsidian3.Plugin {
  async updateFrontMatter(file, entity_props, callback) {
  ...
  async entitySearchCallback(search_term, is_active_note = false) {
    const polite_email = this.settings.politeEmail;
    const emodal = new EntitySuggestionModal(this.app, search_term, polite_email, async (result) => {
  ...
  async onload() {
    ...
    this.addCommand({
      ...
      editorCallback: async (editor, view) => {
        var _a;
        const search_term = (_a = editor.getSelection()) == null ? void 0 : _a.toString();
    ...
    this.addCommand({
      ...
      editorCallback: (editor, view) => {
        var _a;
        const search_term = (_a = this.app.workspace.getActiveFile()) == null ? void 0 : _a.basename.toString();
        if (!search_term) {
          return;
        }
    ...
    this.registerEvent(
      this.app.workspace.on("editor-menu", (menu, editor, view) => {
        menu.addItem((item) => {
          item.setTitle("Link selection to entity").setIcon("document").onClick(async () => {
            const search_term = editor.getSelection();
            this.entitySearchCallback(search_term);
          });
        });
        menu.addItem((item) => {
          item.setTitle("Link active note to entity").setIcon("document").onClick(async () => {
            var _a;
            const search_term = (_a = this.app.workspace.getActiveFile()) == null ? void 0 : _a.basename.toString();
            if (!search_term) {
              return;
...
var EntityLinkerSettingsTab = class extends import_obsidian3.PluginSettingTab {
  ...
  display() {
    ...
    containerEl.empty();
    new import_obsidian3.Setting(containerEl).setName("Polite email").setDesc("Adding email to openalex API requests(for faster and more consistent response times)").addText((text) => text.setPlaceholder("Enter email here").setValue(this.plugin.settings.politeEmail).onChange(async (value) => {
      this.plugin.settings.politeEmail = value;
      await this.plugin.saveSettings();
    }));
    const saveLoc = new import_obsidian3.Setting(containerEl).setName("Entity folder").setDesc("Folder to store entities");
    new FileSuggestionComponent(saveLoc.controlEl, this.app).setValue(this.plugin.settings.entityFolder).setPlaceholder(DEFAULT_SETTINGS.entityFolder).setFilter("folder").setLimit(10).onSelect(async (val) => {
      this.plugin.settings.entityFolder = val.path;
      await this.plugin.saveSettings();
    });
    new import_obsidian3.Setting(containerEl).setName("Overwrite existing properties").setDesc("If checked, existing properties will be overwritten").addToggle((toggle) => toggle.setValue(this.plugin.settings.overwriteFlag).onChange(async (value) => {
